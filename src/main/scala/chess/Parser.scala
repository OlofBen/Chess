...
object Parser:
  ...
  def parse(algebraicNotation: String, pieces : Set[Piece], board : Board): (Position, Position) = 
    if algebraicNotation == "O-O" || algebraicNotation == "0-0" then 
      if board.turn == Color.White then (Position("e1"), Position("g1"))
      else (Position("e8"), Position("g8"))
    else if algebraicNotation == "O-O-O" || algebraicNotation == "0-0-0" then 
      if board.turn == Color.White then (Position("e1"), Position("c1"))
      else (Position("e8"), Position("c8"))
    else
